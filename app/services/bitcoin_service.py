...
class BitcoinService:
    ...
    async def get_bitcoin_price(self) -> Dict[str, Any]:
        ...
        if not COINBASE_SDK_AVAILABLE:
            raise Exception(
                "Coinbase Advanced Python SDK not available. Install with: pip install coinbase-advanced-py"
            )
        ...
        try:
            ...
            try:
                ...
                result = {
                    ...
                    "price_24h_change": (
                        float(ticker.price_percentage_change_24h)
                        if ticker.price_percentage_change_24h
                        else None
                    ),
                    "volume_24h": (
                        float(ticker.volume_24h) if ticker.volume_24h else None
                    ),
                    "market_cap": (
                        float(ticker.market_cap)
                        if hasattr(ticker, "market_cap") and ticker.market_cap
                        else None
                    ),
                    ...
                    "product_info": {
                        ...
                        "status": (
                            product.status if hasattr(product, "status") else None
                        ),
                        "base_currency": (
                            product.base_currency_id
                            if hasattr(product, "base_currency_id")
                            else None
                        ),
                        "quote_currency": (
                            product.quote_currency_id
                            if hasattr(product, "quote_currency_id")
                            else None
                        ),
                    },
                }
                ...
                self.logger.info(
                    f"Authenticated API failed, falling back to public API: {auth_error}"
                )
                ...
                try:
                    ...
                    result = {
                        ...
                        "high_24h": (
                            float(stats_data.get("high", 0))
                            if stats_data.get("high")
                            else None
                        ),
                        "low_24h": (
                            float(stats_data.get("low", 0))
                            if stats_data.get("low")
                            else None
                        ),
                        "open_24h": (
                            float(stats_data.get("open", 0))
                            if stats_data.get("open")
                            else None
                        ),
                        "last_24h": (
                            float(stats_data.get("last", 0))
                            if stats_data.get("last")
                            else None
                        ),
                        "timestamp": datetime.now().isoformat(),
                    ...
                    if result["open_24h"] and result["current_price"]:
                        result["price_24h_change"] = (
                            (result["current_price"] - result["open_24h"])
                            / result["open_24h"]
                        ) * 100
                    ...
                    raise Exception(
                        f"Both authenticated and public API failed. Auth error: {auth_error}. Public error: {public_error}"
                    )
    ...
    async def get_bitcoin_candles(
        self, hours: int = 24, granularity: str = "ONE_HOUR"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Authenticated candles API failed, falling back to public API: {auth_error}"
                )
                ...
                granularity_sec = granularity_seconds.get(
                    granularity, 3600
                )  # Default to 1 hour
