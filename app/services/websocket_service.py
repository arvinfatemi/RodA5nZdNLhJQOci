...
class WebSocketService:
    ...
    def _on_heartbeat(self, msg: dict):
        ...
        self.logger.info(f"Received heartbeat: {json.dumps({k: msg.get(k) for k in ('channel', 'product_id', 'time', 'sequence')}, separators=(',', ':'))}")

    async def start_websocket(self) -> Dict[str, Any]:
        """Start WebSocket connection."""
        if (self.client_instance and 
            hasattr(self.client_instance, '_thread') and 
            self.client_instance._thread and 
            self.client_instance._thread.is_alive()):
