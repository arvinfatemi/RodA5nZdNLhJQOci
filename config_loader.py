...
def _load_google_credentials():
    ...
    if inline:
        try:
            ...
            if info.get("type") == "service_account":
                ...
                log.warning("GCP_SERVICE_ACCOUNT_JSON present but not a service account; ignoring for OAuth flow.")
    ...
    if path and os.path.exists(path):
        try:
            ...
            if "installed" in data or data.get("type") in {"authorized_user", "web"}:
                token_path = os.getenv("GOOGLE_OAUTH_TOKEN_PATH") or os.path.join(os.path.dirname(path), "token.json")
    ...
    client_secrets = os.getenv("GOOGLE_OAUTH_CLIENT_SECRETS") or os.path.join(os.getcwd(), "credentials.json")
    if os.path.exists(client_secrets):
        token_path = os.getenv("GOOGLE_OAUTH_TOKEN_PATH") or os.path.join(os.path.dirname(client_secrets), "token.json")
        try:
